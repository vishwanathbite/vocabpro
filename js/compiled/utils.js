...
const generateSmartDistractors = (correctDef, words, count = 3) => {
  const pool = words.filter(w => w.definition !== correctDef).map(w => w.definition);
...
const buildSynonymPool = (allWords, currentSynonyms) => {
  ...
  return allWords.flatMap(w => w.synonyms || []).filter(syn => !currentSet.has(syn));
};
...
const buildAntonymPool = (allWords, currentAntonyms) => {
  ...
  return allWords.flatMap(w => w.antonyms || []).filter(ant => !currentSet.has(ant));
};
...
const getDailyMotivation = streak => {
  const quotes = ["Every word you learn is a step towards mastery!", "Consistency is the key to vocabulary success.", "Today's practice is tomorrow's confidence.", "Small daily improvements lead to stunning results.", "Your vocabulary is your intellectual toolkit.", "Learning never exhausts the mind.", "Words are the building blocks of expression.", "Each word mastered is a victory achieved.", "The limits of your language are the limits of your world.", "Knowledge of words is knowledge of things."];
