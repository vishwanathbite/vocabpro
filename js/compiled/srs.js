...
const calculateNextReview = (entry, quality) => {
  ...
  newEntry.easeFactor = Math.max(1.3, Math.min(2.5, newEntry.easeFactor + (0.1 - (5 - quality) * (0.08 + (5 - quality) * 0.02))));
...
const SRSManager = {
  ...
  getDueWords: (words, limit = 10) => {
    ...
    return scoredWords.sort((a, b) => b.score - a.score).slice(0, limit).map(item => item.word);
  },
  /**
