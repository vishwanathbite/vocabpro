...
const SettingsManager = {
  ...
  defaults: {
    ...
    speechEnabled: true,
    darkMode: true,
    // App is always dark mode by design
...
const QuizHistoryManager = {
  ...
  addQuiz: quizData => {
    ...
    const entry = {
      ...
      accuracy: quizData.questionsTotal > 0 ? Math.round(quizData.questionsCorrect / quizData.questionsTotal * 100) : 0,
  ...
  getStats: () => {
    ...
    stats.averageAccuracy = stats.totalQuestions > 0 ? Math.round(stats.totalCorrect / stats.totalQuestions * 100) : 0;
...
const OnboardingManager = {
  ...
  getStatus: () => {
    if (typeof StorageManager !== 'undefined') {
      const state = StorageManager.loadState();
      return {
        ...OnboardingManager.defaultStatus,
        ...state.onboarding
      };
    }
...
const KeyboardShortcuts = {
  quiz: {
    ...
    'Enter': {
      action: 'next',
      description: 'Next question / Continue'
    },
    ' ': {
      action: 'next',
      description: 'Next question / Continue'
  ...
  flashcard: {
    ...
    'ArrowRight': {
      action: 'know',
      description: 'Mark as known'
    },
    'ArrowLeft': {
      action: 'dontKnow',
      description: 'Mark as learning'
    },
    'k': {
      action: 'know',
      description: 'Mark as known'
