...
const DailyGoalsManager = {
  ...
  updateProgress: (questionsAdded, pointsAdded) => {
    ...
    if (data.history[todayKey].questionsAnswered >= goal.questions || data.history[todayKey].pointsEarned >= goal.points) {
  ...
  getWeekHistory: () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      history.push({
        date: checkDate,
        dayName: checkDate.toLocaleDateString('en-US', {
          weekday: 'short'
        }),
